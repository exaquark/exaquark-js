...
class PeerLink {
  constructor (iid, audioPlayer, videoPlayer) {
...
webrtc.startP2P = function (iid, isInitiator) {
  ...
  if (webrtc.mediaWanted.video) {
    selfPeerLink.peerConnection.addTrack(
      webrtc.mediaSelected.video.stream.getVideoTracks()[webrtc.mediaSelected.video.trackNumber],
      webrtc.mediaSelected.video.stream)
  }
  if (webrtc.mediaWanted.audio) {
    selfPeerLink.peerConnection.addTrack(
      webrtc.mediaSelected.audio.stream.getAudioTracks()[webrtc.mediaSelected.audio.trackNumber],
      webrtc.mediaSelected.audio.stream)
  }
...
webrtc.onwebrtcsignal = function (sourceIid, incomingSignal) {
  ...
  if (incomingSignal.sdp) {
    ...
    if (incomingSignal.sdp.type === 'offer') {
      ...
      webrtc.peerLink[sourceIid].peerConnection.setRemoteDescription(incomingSignal.sdp)
        .then(function () {
    ...
    webrtc.peerLink[sourceIid].peerConnection.addIceCandidate(incomingSignal.candidate)
    .catch(function (error) {
